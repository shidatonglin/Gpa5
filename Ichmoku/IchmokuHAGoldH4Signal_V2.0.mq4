...
string pairs[];
string suggestion_buy[];
string suggestion_sell[];
int index_buy=0,index_sell=0;
//---

int init() {
  //getAvailableCurrencyPairs(pairs);
  ReadSymbolsFromFile(pairs,FILENAME);
...
int start(){
  ...
  printArray(suggestion_buy,suggestion_sell);
  return(0);
}
...
void checkForOpen(string name){
  ...
  if(signal == 1) {
   suggestion_buy[index_buy]=name;
   index_buy++;
  }//open_buy(name);
  else if(signal == -1){
   suggestion_sell[index_sell]=name;
   index_sell++;
  } //open_sell(name);
}

void printArray(string &buyArray[],string &sellArray[]){
   string text = "Buy suggestion: \n";
   
   for(int i=0;i<ArraySize(buyArray);i++){
      text = text + buyArray[i]+ ",";
      if(i==10 || i==20 || i==30|| i==40|| i==50|| i==60|| i==70)text = text + "\n";
   }
   text = text + "Sell suggestion: \n";
   for(int i=0;i<ArraySize(sellArray);i++){
      text = text + sellArray[i]+ ",";
      if(i==10 || i==20 || i==30|| i==40|| i==50|| i==60|| i==70)text = text + "\n";
   }
   Comment("start to print array:"+text);
}
...
int ReadSymbolsFromFile(string& temporary_symbols[],string file_name="All.txt")
  {
...
//--- Open the file for reading from the common folder of the terminal
   int file_handle=FileOpen(file_name, FILE_READ|FILE_WRITE|FILE_TXT, "/t");
...
int getIchomuTrendSignal(string name){
  ...
  Print("IchomuC-->"+IchomuC);
  Print("name-->"+name);
  Print("IchomuD-->"+IchomuD);
  // HA close value
  double haClose = iCustom(name, Low_TF, "Heiken Ashi", 0,0,0,0, 3, shift);
   Print("haClose-->"+haClose);
