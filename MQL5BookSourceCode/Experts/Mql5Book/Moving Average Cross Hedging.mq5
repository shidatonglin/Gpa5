...
void OnTick()
{
	...
	if(newBar == true)
	{
		...
		if(FastMA.Main(barShift) > SlowMA.Main(barShift) && glBuyTicket == 0)
		{
			...
			Trade.CloseHedge(glSellTicket);
			glSellTicket = 0;
			
			// Open buy position
			glBuyTicket = Trade.BuyHedge(_Symbol,tradeSize);
			...
			if(glBuyTicket > 0)  
			{
				...
				if(buyStop > 0 || buyProfit > 0) Trade.ModifyHedge(glBuyTicket,buyStop,buyProfit);
			} 
		}
		...
		{
			Trade.CloseHedge(glBuyTicket);
			glBuyTicket = 0;
			
			glSellTicket = Trade.SellHedge(_Symbol,tradeSize);
			...
			if(glSellTicket > 0)
			{
				...
				if(sellStop > 0 || sellProfit > 0) Trade.ModifyHedge(glSellTicket,sellStop,sellProfit);
			} 
		}
